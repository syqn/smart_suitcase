...
#define APP_NAME "DS TWR INIT TEST"
...
static uint8 tx_poll_msg[] = {0x41, 0x88, 0, 0xCA, 0xDE, 'S', 'O', 'F', 'A', 0x21, 0, 0};
static uint8 rx_resp_msg1[] = {0x41, 0x88, 0, 0xCA, 0xDE, 'F', 'A', 'S', '1', 0x10, 0x02, 0, 0, 0, 0};
static uint8 rx_resp_msg2[] = {0x41, 0x88, 0, 0xCA, 0xDE, 'F', 'A', 'S', '2', 0x10, 0x02, 0, 0, 0, 0};
static uint8 rx_resp_msg3[] = {0x41, 0x88, 0, 0xCA, 0xDE, 'F', 'A', 'S', '3', 0x10, 0x02, 0, 0, 0, 0};
static uint8 tx_final_msg[] = {0x41, 0x88, 0, 0xCA, 0xDE, 'S', 'O', 'F', 'A', 0x23,\
...
#define FINAL_MSG_S1_POLL_TX_TS_IDX 10
#define FINAL_MSG_S1_RESP_RX_TS_IDX 14
#define FINAL_MSG_S1_FINAL_TX_TS_IDX 18
#define FINAL_MSG_S2_POLL_TX_TS_IDX 22
#define FINAL_MSG_S2_RESP_RX_TS_IDX 26
#define FINAL_MSG_S2_FINAL_TX_TS_IDX 30
#define FINAL_MSG_S3_POLL_TX_TS_IDX 34
#define FINAL_MSG_S3_RESP_RX_TS_IDX 38
#define FINAL_MSG_S3_FINAL_TX_TS_IDX 42
...
#define RESP_RX_S3_TO_FINAL_TX_DLY_UUS 8000//3100
/* Receive response timeout. See NOTE 5 below. */
#define RESP_RX_TIMEOUT_UUS 20000//2700
/* Preamble timeout, in multiple of PAC size. See NOTE 6 below. */
#define PRE_TIMEOUT 512

/* Time-stamps of frames transmission/reception, expressed in device time units.
 * As they are 40-bit wide, we need to define a 64-bit int type to handle them. */
typedef unsigned long long uint64;
static uint64 poll_tx_ts_s1;
static uint64 resp_rx_ts_s1;
//static uint64 final_tx_ts_s1;
static uint64 poll_tx_ts_s2;
static uint64 resp_rx_ts_s2;
//static uint64 final_tx_ts_s2;
static uint64 poll_tx_ts_s3;
static uint64 resp_rx_ts_s3;
...
//    dwt_setleds(DWT_LEDS_ENABLE);
    ...
    while (1)
    {
		...
		if (status_reg & SYS_STATUS_RXFCG)
		{
			...
			frame_len3 = dwt_read32bitreg(RX_FINFO_ID) & RX_FINFO_RXFLEN_MASK;
			...
			if (frame_len3 <= RX_BUF_LEN)
			{
				dwt_readrxdata(rx_buffer, frame_len3, 0);
				EMBARC_PRINTF("\r\n The rx_buffer3 is %x,%x,%d,%x,%x,%c%c%c%c,%x,%d,%d,%d,%d,%d\r\n",\
			...
			* As the sequence number field of the frame is not relevant, it is cleared to simplify the validation of the frame. */
			...
			{
				uint32 final_tx_time3;

				/* Retrieve poll transmission and response reception timestamp. */
				poll_tx_ts_s3 = get_tx_timestamp_u64();
				resp_rx_ts_s3 = get_rx_timestamp_u64();

				/* Compute final message transmission time. See NOTE 10 below. */
				final_tx_time3 = (resp_rx_ts_s3 + (RESP_RX_S3_TO_FINAL_TX_DLY_UUS * UUS_TO_DWT_TIME)) >> 8;
				dwt_setdelayedtrxtime(final_tx_time3);

				/* Final TX timestamp is the transmission time we programmed plus the TX antenna delay. */
				final_tx_ts = (((uint64)(final_tx_time3 & 0xFFFFFFFEUL)) << 8) + TX_ANT_DLY;

				/* Write all timestamps in the final message. See NOTE 11 below. */
				//将说有的时刻数据写入发送包，用于设备B计算
				final_msg_set_ts(&tx_final_msg[FINAL_MSG_S3_POLL_TX_TS_IDX], poll_tx_ts_s3);
				final_msg_set_ts(&tx_final_msg[FINAL_MSG_S3_RESP_RX_TS_IDX], resp_rx_ts_s3);
				final_msg_set_ts(&tx_final_msg[FINAL_MSG_S3_FINAL_TX_TS_IDX], final_tx_ts);
